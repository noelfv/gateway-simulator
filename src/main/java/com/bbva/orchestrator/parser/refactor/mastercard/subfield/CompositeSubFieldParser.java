...
public class CompositeSubFieldParser implements FieldParserStrategy {
     ...
     */
    public Map<String, String> parseToMap(String rawDataSegment, IFieldDefinition fieldDefinition) {
        Map<String, String> parsedSubFieldsMap = new LinkedHashMap<>();
        int currentOffset = 0;

        // Caso especial: subcampo fijo 48.01
        if ("48".equals(compositeFieldId)) {
            ISOSubField48Mastercard sf01Def = subFieldDefinitions.get("01");
            if (sf01Def != null && !sf01Def.isVariable() && sf01Def.getLength() > 0) {
                ...
                if (currentOffset + lenHex <= rawDataSegment.length()) {
                    ...
                    ParsedFieldResult result = sf01Def.getParserStrategy().parse(valueHex, lenHex, sf01Def);
        ...
        while (currentOffset < rawDataSegment.length()) {
            ...
            ISOSubField48Mastercard subFieldDef = subFieldDefinitions.get(tag);
            if (subFieldDef == null) {
