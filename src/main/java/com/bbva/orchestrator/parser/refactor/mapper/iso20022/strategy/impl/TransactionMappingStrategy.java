...
public class TransactionMappingStrategy implements SectionMappingStrategy<TransactionDTO> {
    ...
    public TransactionDTO map(ISO8583 input, Map<String, String> subFields) {
        ...
        ReconciliationAmountDTO reconciliationAmount = ReconciliationAmountDTO.builder()
                ...
                .effectiveExchangeRate(fieldService.conversionRateValidationBig(input.getConversionRateSettlement()))
        ...
        CardholderBillingAmountDTO cardholderBillingAmount = CardholderBillingAmountDTO.builder()
                ...
                .effectiveExchangeRate(fieldService.conversionRateValidationBig(input.getConversionRate()))
    ...
    public void translate(ISO20022 input, ISO8583.ISO8583Builder iso8583Builder) {
        ...
        TransactionDTO transaction = input.getTransaction();
        TransactionIdDTO transactionId = transaction.getTransactionId();
        AccountFromDTO accountFromDTO = transaction.getAccountFrom();
        AccountToDTO accountToDTO = transaction.getAccountTo();
        TransactionAmountsDTO transactionAmountsDTO= transaction.getTransactionAmounts();
        List<AdditionalFeesDTO> listAdditionalFeesDTO= transaction.getAdditionalFee();
        List<AdditionalAmountDTO> listAdditionalAmountDTOS= transaction.getAdditionalAmount();
        List<AdditionalDataDTO> listAdditionalDataDTOS= transaction.getAdditionalData();

        iso8583Builder
                .retrievalReferenceNumber(fieldService.getFieldValue(transactionId, TransactionIdDTO::getRetrievalReferenceNumber, DEFAULT_EMPTY_VALUE))
                .transmissionDateTime(fieldService.getFieldValue(transactionId, TransactionIdDTO::getTransmissionDateTime, DEFAULT_EMPTY_VALUE))// Se espera el formato YYYYMMDDHHMMSS pero se devuleve en este formato YYYY-MM-DDT24HH:MM:SS.000Z
                .systemTraceAuditNumber(fieldService.getFieldValue(transactionId , TransactionIdDTO::getSystemTraceAuditNumber, DEFAULT_EMPTY_VALUE))
                .localTransactionDate(fieldService.getFieldValue(transactionId , TransactionIdDTO::getLocalDate, DEFAULT_EMPTY_VALUE))
                .localTransactionTime(fieldService.getFieldValue(transactionId , TransactionIdDTO::getLocalTime, DEFAULT_EMPTY_VALUE));
