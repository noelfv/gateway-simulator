...
public class OrchestratorFlowProcess implements IParser {
    ...
    public ISO20022 convert8583to20022(String originalMessage) {
        ...
        if(isFlowAsynchronousResponse(iso20022)) {
            String x= extractMessageOriginal(iso20022);
        }else{
            if(!isCustomDataLocalPresent(iso20022)){
               String y= extractMessageOriginal(iso20022);
            }else{
                //TODO Este bloque solo genera la trama de respuesta para el flujo sincrono
                Map<String, String> fieldsValuesUnMapper = delegateMapper.unMapper(iso20022);
                Map<String, String> fieldsValuesResponse=  ISO8583Builder.logicResponse(fieldsValuesUnMapper);
                String originalMessageTemp= delegateParser.unParser(fieldsValuesResponse);
                String x=originalMessageTemp;
            }
        }
        return iso20022;
    }
